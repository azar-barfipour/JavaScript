...
const str = 'please all passengers come to the door and enter from door for boarding.'
console.log(str.replaceAll('door','gate')); 


//splice split , join method
...
capitalizeNames('ms. azar barfipour')
...
document.querySelector('button').addEventListener('click',function(){
  ...
  for(const [i,row] of rows.entries()){
    ...
    // newRow.push(splited[0] + splited[1][0].toUpperCase() + splited[1].slice(1));
  }
...
for(const flight of newFlight){
  const flightItems = flight.split(';');
  const [a,b,c,d] = flightItems;
  console.log(`${a.startsWith('_Delayed') ? '🔴' : ''}${a.replaceAll('_',' ')} from ${b.slice(0,3).toUpperCase()} to ${c.slice(0,3).toUpperCase()} (${d.replace(':','h')})`.padStart(45));
}